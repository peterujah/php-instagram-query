...
class userAgent {
     /**
     * Hold selected windows operating system 
     * @var string $device
    */
    ...
    */
    ...
    public function getOS(string $os = '') {
        $_os = [];
        if($os === '' || in_array($os, [ 'chrome', 'firefox', 'explorer' ])) {
    ...
    public function generate(): string 
    {
        if($this->device === '') {
            $r = random_int(0, 100);
            if($r >= 44) {
                $this->device = array_rand([ 'firefox' => 1, 'chrome' => 1, 'explorer' => 1 ]);
            } else {
                $this->device = array_rand([ 'iphone' => 1, 'android' => 1, 'mobile' => 1 ]);
            }
        } elseif($this->device == 'windows' || $this->device == 'mac' || $this->device == 'linux') {
            $agents = [ 'firefox' => 1, 'chrome' => 1 ];
            if($this->device == 'windows') {
                $agents['explorer'] = 1;
            }
            $this->device = array_rand($agents);
        }
  
        if($this->device == 'chrome') {
            return 'Mozilla/5.0 (' . $this->getOS($this->device) . ') AppleWebKit/' . (random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603)) . '.' . random_int(1, 50) . ' (KHTML, like Gecko) Chrome/' . self::chromeVersion([ 'min' => 47,'max' => 55 ]) . ' Safari/' . (random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603));
        }

        if($this->device == 'firefox') {
            return 'Mozilla/5.0 (' . $this->getOS($this->device) . ') Gecko/' . (random_int(1, 100) > 30 ? '20100101' : '20130401') . ' Firefox/' . self::firefoxVersion([ 'min' => 45,'max' => 74 ]);
        }
        
        if($this->device == 'explorer') {
            return 'Mozilla / 5.0 (compatible; MSIE ' . ($int = random_int(7, 11)) . '.0; ' . $this->getOS('windows') . ' Trident / ' . ($int == 7 || $int == 8 ? '4' : ($int == 9 ? '5' : ($int == 10 ? '6' : '7'))) . '.0)';
        }
        
        if($this->device === 'mobile' || $this->device === 'android' || $this->device === 'iphone' || $this->device === 'ipad' || $this->device === 'ipod') {
            return 'Mozilla/5.0 (' . $this->getMobileOS($this->device) . ') AppleWebKit/' . (random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603)) . '.' . random_int(1, 50) . ' (KHTML, like Gecko)  Chrome/' . self::chromeVersion([ 'min' => 47,'max' => 55 ]) . ' Mobile Safari/' . (random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603)) . '.' . random_int(0, 9);
        } 
        
        return "Mozilla/5.0 (Windows NT 10.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/40.0.2214.93 Safari/537.36";
